...
router.put("/leaves", passport.authenticate('jwt', {session: true}), function(req, res, next) {
  ...
  new Promise((resolve, reject)=>{
    user.updateLeave(id, status).then((data)=>{
      notifications.sendSpecificDeviceNotification(username, "admin", data.employee_id, {title: `Leave request ${status?'approved':'declined'}`, 'message': `Your leave request for ${data.number_of_days} day(s) with subject ${data.reason} has been ${data.approved?'approved!':'declined!'}`})
      resolve(data)
    }).catch((err)=>{
      reject(err)
    })
